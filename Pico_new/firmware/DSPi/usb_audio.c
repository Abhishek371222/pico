...
volatile AudioState audio_state = { .freq = 96000 };
...
volatile uint32_t pending_rate = 96000;
...
#endif
struct audio_format audio_format_48k = { .format = AUDIO_BUFFER_FORMAT_PCM_S32, .sample_freq = 96000, .channel_count = 2 };
        ...
        switch (setup->bRequest) {
            ...
            case REQ_GET_STATUS: {
                ...
                switch (setup->wValue) {
                    ...
                    case 14: { uint32_t vsel = (vreg_and_chip_reset_hw->vreg >> VREG_AND_CHIP_RESET_VREG_VSEL_LSB) & 0xFu; resp = 550u + vsel * 50u; } break;  // Core voltage in mV
...
void usb_sound_card_init(void) {
    ...
    dsp_recalculate_all_filters(96000.0f);
