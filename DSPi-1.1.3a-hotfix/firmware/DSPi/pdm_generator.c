...
// At 96kHz the chunk rate doubles to 768kHz, shifting the effective corner to ~16kHz — still above audible range.
...
static void pdm_processing_loop() {
    ...
    while (core1_mode == CORE1_MODE_PDM || fade_out_pos > 0) {
        ...
        // Rate-adaptive window: always 1ms regardless of sample rate.
        // factor 26 ≈ 25600/1000 converts µs-active-per-ms into Q8 percentage.
        if (sample_counter >= (audio_state.freq / 1000)) {
            uint32_t inst_q8 = (active_us_accumulator * 26);
